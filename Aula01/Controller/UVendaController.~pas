...
type
  TVendaController = class
     public
        ...
        function PesquisaVenda (pDataVenda : String ) : TColVenda;
...
function TVendaController.PesquisaVenda(pDataVenda: String): TColVenda;
var
   xVendaDAO : TVendaDAO;
   xCondicao : string;
begin
   try
      try
         ...
         xCondicao :=
            IfThen(pDataVenda <> EmptyStr,
               'WHERE                                         '#13+
               '        (DATAVENDA LIKE UPPER (''%'+ pDataVenda +'%'')) '#13+
               'ORDER BY DATAVENDA, ID', EmptyStr);
